    ...
    if (isMultiSelect) {
      if (selectedItemIndexes.includes(index)) {
        if (selectedItemIndexes.length > 1)
          selectedItemIndexes = selectedItemIndexes.filter((i) => i !== index);
...
<div class="select">
  {#if LabelComponent}
    <svelte:component
      this={LabelComponent}
      selectedItems={list.filter((_, index) =>
  ...
  {:else}
    <div
      class="label"
      class:rounded_light={isRounded === undefined
        ? $_theme.isRounded
        : isRounded}
    ...
    >
      ...
      <svg
        ...
        class="svg-arrow svelte-qu6obu"
        ><path
          d="M3.33337 6L8.00004 10L12.6667 6"
          stroke="#BABABA"
          stroke-width="2"
      ...
      >
        <div class="list__container paper" bind:this={listContainer}>
          {#each list as item, index}
            <div
              class="item"
              class:selected={selectedItemIndexes.includes(index)}
