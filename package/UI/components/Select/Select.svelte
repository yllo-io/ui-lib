...
<div class="select">
    ...
    {:else}
        ...
        >
            <div class="label__value color_line6 noselect">{@html list[selectedItemIndexes[0]].value}</div>
                        ...
                        <div class="item" class:selected={selectedItemIndexes.includes(index)} on:click={() => onSelect(index)}>
                            ...
                            {:else}
                                <div
                                    ...
                                    use:interactiveElement={{ isActive: $_theme.isInteractiveCursor && isActive, isCursorHover: false }}
                                >
                                    {@html item.value}
                                </div>
                            {/if}
