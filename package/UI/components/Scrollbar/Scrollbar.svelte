<script>
    ...
    let thumbHeight = 0, thumbYPos = 0, minThumbHeight = 3;
    ...
    $: {
        ...
        const scrollTopFixed = minThumbHeight > thumbHeight ? scrollTop * ((100 - (minThumbHeight - thumbHeight)) / 100 - paddingFix) : scrollTop * (1 - paddingFix);
        thumbYPos = Math.min(Math.max(Math.min(scrollTopFixed, contentHeight - wrapperHeight), 0) / contentHeight * 100, 100 - minThumbHeight);
    }

    let visibleTimeout, visible = hideNotActive ? false : true;
...
<div class:visible={visible && contentHeight > wrapperHeight} class="scrollbar" style="right: {offsetRight}px; height: calc(100% - {paddingBottom}px);" on:mousedown={OnTrackMousedown} on:mouseleave={CancelTrackScrolling} on:mouseup={CancelTrackScrolling}>
    <div class="thumb" class:smooth id="{ThumbId}" on:mousedown|stopPropagation={OnThumbMousedown} class:holding style="height: {thumbHeight}%; top: {thumbYPos}%; min-height: {minThumbHeight}%;">
    </div>
</div>